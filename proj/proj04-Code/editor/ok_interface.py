...
class AreDifferent(
    PromptOutput, namedtuple("AreDifferent", ["prompt", "expected", "actual"])
):
    def representation(self):
        return "{expected}\n{actual}\n{prompt}".format(
            prompt=self.prompt,
...
class Error(PromptOutput, namedtuple("PromptOutput", ["prompt", "error"])):
    def representation(self):
        return "{error}\n{prompt}".format(
            error=pad("; Error: ", ";", self.error), prompt=self.prompt
        )
...
class Same(PromptOutput, namedtuple("Same", ["prompt", "output"])):
    def representation(self):
        return "{output}\n{prompt}".format(
            prompt=self.prompt, output=pad("; Success: ", ";", self.output)
        )
...
class TestCaseResult(
    namedtuple("TestCaseResult", ["cases_passed", "cases_out", "setup_out"])
):
    ...
    @property
    def dictionary(self):
        return {"code": self.output, "passed": self.success}
...
def process(output, success):
    ...
    if not success:
        try:
            expected_index = next(
                idx for idx, line in enumerate(lines) if "# Error: expected" in line
            )
            but_got_idx = next(
                idx for idx, line in enumerate(lines) if "# but got" in line
            )
...
def process_case(case):
    ...
    return TestCaseResult(
        interpret_success_overall, interpret_out_overall, process(setup_out, True)
    )


def reload_tests():
    for testname in filter(
        lambda x: x.lower().endswith(".py"), os.listdir(os.curdir + "/tests")
    ):
...
if __name__ == "__main__":
    ...
    sys.path.insert(
        0,
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "ok"),
    )
    try:
        output = run_tests()
    finally:
