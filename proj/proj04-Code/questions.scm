...
(define (enumerate s)
  ; BEGIN PROBLEM 15
  (define (helper index remainder)
    (if (null? remainder)
      nil
      (cons (list index (car remainder)) (helper (+ index 1) (cdr remainder)))
    )
  )
  (helper 0 s))
  ; END PROBLEM 15
         ...
         (let ((values (cadr expr))
           ...
           (let ((zipped (zip values)))
             (let ((params (car zipped))
                   (args (map let-to-lambda (cadr zipped))))
               (let ((new-body (map let-to-lambda body)))
                 (cons (cons 'lambda (cons params new-body)) args))))
           ; END PROBLEM 17
           ))
        (else
         ; BEGIN PROBLEM 17
