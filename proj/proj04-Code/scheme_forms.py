...
def do_cond_form(expressions: Pair, env: Frame):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            return eval_all(clause.rest, env) if clause.rest else test
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings: Pair, env: Frame):
    ...
    while bindings is not nil:
        ...
        names = Pair(bindings.first.first, names) # type: ignore
        values = Pair(scheme_eval(bindings.first.rest.first, env), values) # type: ignore
        bindings = bindings.rest
    # END PROBLEM 14
    return env.make_child_frame(names, values) # type: ignore


def do_define_macro(expressions: Pair, env: Frame):
    ...
    validate_form(expressions, 2)
    signature = expressions.first
    if not (isinstance(signature, Pair) and scheme_symbolp(signature.first)):
        raise SchemeError("bad macro form")
    formals = signature.rest
    validate_formals(formals)
    macro = MacroProcedure(formals, expressions.rest, env)
    env.define(signature.first, macro)
    return signature.first
