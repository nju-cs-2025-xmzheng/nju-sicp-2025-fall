...
# a dirty hack to make builtins work
# in fact, functions like `map`, `filter`, `reduce` should be implemented in scheme
# so we need not to use `apply` in python code
# use `apply` in scheme code is ok
def strict_scheme_apply(fn, args, env):
    res = scheme_apply(fn, args, env)
    if isinstance(res, Unevaluated):
        return scheme_eval(res.expr, res.env)
    else:
        return res
...
def scheme_map(fn, s, env):
    ...
    return s.map(lambda x: strict_scheme_apply(fn, Pair(x, nil), env))
...
def scheme_filter(fn, s, env):
    ...
    while s is not nil:
        ...
        if strict_scheme_apply(fn, Pair(item, nil), env):
            if head is nil:
                head = Pair(item, nil)
                current = head
            else:
...
def scheme_reduce(fn, s, env):
    ...
    while s is not nil:
        value = strict_scheme_apply(fn, scheme_list(value, s.first), env)
        s = s.rest
    return value


@builtin("load", expect_env=True)
...
@builtin("load-all", expect_env=True)
        ...
        raise SchemeError(
            "\n".join(
                [
                    ...
                    "    python -m pip install 'abstract_turtle[pillow_canvas]'",
