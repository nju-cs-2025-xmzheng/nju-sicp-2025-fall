...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        outcome = dice()
        if (outcome == 1): has_one = 1
        sum += outcome
    return 1 if has_one else sum
    # END PROBLEM 1
...
def play(strategy0, strategy1,
    ...
    while score0 < goal and score1 < goal:
        if (who == 0):
            ...
            if (swine_swap(score0)): score0, score1 = score1, score0
        else:
            score1 += take_turn(strategy1(score1, score0), score0, dice)
            if (swine_swap(score1)): score0, score1 = score1, score0
        who = 1 - who
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL_SCORE):
    ...
    for i in range(100):
        for j in range(100):
            if strategy(i, j) != num: return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_score = 0
    for i in range(1, 11):
        current_score = make_averaged(roll_dice, total_samples)(i, dice)
...
def swine_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if swine_swap(score + picky_piggy(opponent_score)):
        return 0 if opponent_score - score >= threshold else num_rolls
    return 0 if picky_piggy(opponent_score) >= threshold else num_rolls
    # END PROBLEM 11
