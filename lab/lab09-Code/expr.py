...
class CallExpr(Expr):
    ...
    """
    def __init__(self, operator: Expr, operands: list[Expr]):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        return self.operator.eval(env).apply(list(map(lambda x: x.eval(env), self.operands)))
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for (param, value) in zip(self.parameters, arguments):
            env[param] = value
